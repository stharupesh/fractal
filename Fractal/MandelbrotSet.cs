...
namespace Fractal
{
    ...
    {
        public MandelbrotSet() // app reference
        {
        ...
        {
            double r = 0.0, i = 0.0, m = 0.0; // r= real, i = imaginary and m = mandelbrot set, used for storing absolute value or distance
            ...
            while ((j < max) && (m < 4.0)) // while (( j is less than the max iteration) && (the output of the f(x) is inside the radius of 2 / inside mandelbrot set))
            {
                ...
                m = r * r - i * i; // x^2 - y^2
                i = 2.0 * r * i + ywert; // 2xy + c where c is constant
                r = m + xwert; // xwert is constant
            }
